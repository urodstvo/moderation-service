...
func (r *repository) GetById(ctx context.Context, id int) (gomodels.TaskGroup, error) {
	conn := r.getter.DefaultTrOrDB(ctx, r.db)

	query, args, err := sq.Select("*").From("task_groups").Where(squirrel.Eq{"id": id}).Where(squirrel.Expr("deleted_at IS NULL")).ToSql()
	if err != nil {

...
import (
	...
	grpcimpl "github.com/urodstvo/moderation-service/apps/task/internal/grpc-impl"
...
func New(opts Opts) error {
	grpcNetListener, err := net.Listen("tcp", fmt.Sprintf("0.0.0.0:%d", constants.TASK_SERVER_PORT))
	if err != nil {
		return err
	}

	grpcServer := grpc.NewServer(grpc.StatsHandler(otelgrpc.NewServerHandler()))
	task.RegisterTaskServiceServer(grpcServer, opts.Service)

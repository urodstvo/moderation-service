...
func (r *repository) GetByEmail(ctx context.Context, email string) (gomodels.User, error) {
	...
	query, args, err := sq.Select("*").From("users").Where(squirrel.Eq{"email": email}).ToSql()
	if err != nil {
		return gomodels.User{}, fmt.Errorf("failed to build query: %w", err)
	}

	user := gomodels.User{}
	err = conn.QueryRow(ctx, query, args...).Scan(&user.Id, &user.Email, &user.Password, &user.CreatedAt, &user.UpdatedAt, &user.IsVerified, &user.Role)
	if err != nil {

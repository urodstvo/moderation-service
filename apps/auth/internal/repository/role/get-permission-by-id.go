...
func (r *repository) GetPermissionById(ctx context.Context, id int) (gomodels.Permission, error) {
	conn := r.getter.DefaultTrOrDB(ctx, r.db)

	query, args, err := sq.Select("*").From("permissions").Where(squirrel.Eq{"id": id}).ToSql()
	if err != nil {

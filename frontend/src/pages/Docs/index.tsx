...
import TextModeration from "./Tabs/TextModeration";
...
export const Docs = () => {
    ...
    const openGroup: MouseEventHandler = (e) => {
        ...
        .closest('.' + styles.sidebarListContainer)?.classList.toggle(styles.opened)
    }
    ...
    useEffect(() => {
        const tabs = (list.current as HTMLDivElement).querySelectorAll('[data-tab]');
        tabs.forEach(element => {element.classList.remove(styles.sidebarTabSelected)});
        
        (list.current as HTMLDivElement).querySelector(`[data-tab="${selectedTab}"]`)?.classList.add(styles.sidebarTabSelected);
    }, [selectedTab])
    ...
    return (
        ...
        <div className={styles.sidebarContainer}>
            <div className={styles.sidebarContent} ref={list}>
                <div className={styles.sidebarTab} data-tab={1} onClick={changeTab}>Getting started</div>
                <div className={styles.sidebarTab} data-tab={2} onClick={changeTab}>REST API</div>
                ...
                <div className={styles.sidebarTab} data-tab={3} onClick={changeTab}>FAQ</div>
            </div>
        </div>

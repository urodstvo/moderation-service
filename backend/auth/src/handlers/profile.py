...
async def change_role(request: Request, role: str, db: AsyncSession):
    ...
    match role:
        case 'student':
            await ProfilesTable.updateProfile(ProfileModel(user_id=user_id), ProfileTable(role=role), db)
            return JSONResponse(content='Role changed')
        case 'company':
            await ProfilesTable.updateProfile(ProfileModel(user_id=user_id), ProfileTable(is_company_requested=True), db)

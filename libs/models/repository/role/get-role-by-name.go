...
func (r *repository) GetRoleByName(ctx context.Context, role string) (gomodels.Role, error) {
	conn := r.getter.DefaultTrOrDB(ctx, r.db)

	query, args, err := sq.Select("*").From("roles").Where(squirrel.Eq{"name": role}).ToSql()
	if err != nil {
